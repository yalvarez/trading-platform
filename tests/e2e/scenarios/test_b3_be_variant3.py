...
from tests.e2e.scenarios import b3_be_variant3, b1_be_variant1
...
@pytest.mark.asyncio
async def test_b3_sends_variant_message_and_confirms_sl_moved_to_entry():
    ctx = _ctx_with_open_position()

    result = await b3_be_variant3.run(ctx)

    sent_texts = [c.args[1] for c in ctx.sender.send.await_args_list]
    assert "lock all your trades in Break Even" in sent_texts
    assert result.outcome == ScenarioOutcome.PASS
    assert result.name == "b3_be_variant3"


@pytest.mark.asyncio
async def test_b3_sets_shared_message_constant_on_b1_before_delegating():
    ctx = _ctx_with_open_position()

    await b3_be_variant3.run(ctx)

    assert b1_be_variant1.MESSAGE == "lock all your trades in Break Even"
