...
async def test_positions_for_symbol_returns_position_dicts(monkeypatch):
    ...
    monkeypatch.setattr(
        "tests.e2e.vps_observer.build_mt5_client",
...
async def test_positions_for_symbol_calls_build_mt5_client_with_host_and_port(monkeypatch):
    ...
    monkeypatch.setattr("tests.e2e.vps_observer.build_mt5_client", fake_build)
    observer = VpsObserver(redis_client=MagicMock(), mt5_host="mt5_acct1", mt5_port=8001)
...
@pytest.mark.asyncio
async def test_positions_for_symbol_propagates_failure(monkeypatch):
    fake_client = MagicMock()
    fake_client.positions_get.side_effect = RuntimeError("connection error")
    monkeypatch.setattr(
        "tests.e2e.vps_observer.build_mt5_client",
        lambda host, port: fake_client,
    )
