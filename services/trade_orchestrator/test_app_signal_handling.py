...
async def test_fast_signal_runner_still_gets_be_and_trailing_when_full_signal_never_arrives():
    ...
    full signal? Yes — tp1_leg's temporary TP and a synthetic tp2 give the
    runner a valid unit > 0 from the start, so _apply_trailing's tp1/tp2/
    entry_price != None guard doesn't block it.

    Unlike before 2026-09-09, `unit` no longer cancels out of the SL
    formula — the offset is now scaled by (tp1_price - entry_price), not by
    unit (see _apply_trailing's docstring for why). So this test computes
    the expected SL directly from the fast signal's actual synthesized
    tp1_price/entry_price rather than asserting a value independent of
    them.
    """
    from services.trade_orchestrator.app import handle_signal_fields
    ...
    # the full-signal path (SL = entry + multiple*(tp1-entry)), not stay frozen at BE.
    advance = 30.0
    sim.price = runner_leg.tp1_price + advance
    sim.positions[runner_leg.ticket]['price_current'] = sim.price
    await tm._tick_once_account(ACCOUNTS[0])

    unit = runner_leg.tp2_price - runner_leg.tp1_price
    multiple = advance / unit
    entry_to_tp1 = runner_leg.tp1_price - runner_leg.entry_price
    expected_sl = runner_leg.entry_price + multiple * entry_to_tp1
    runner_pos = sim.positions_get(ticket=runner_leg.ticket)[0]
