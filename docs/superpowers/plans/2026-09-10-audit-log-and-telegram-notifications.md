...
**Interfaces:**
- Consumes: `SimuladorMT5.close_position_by_sl`/`close_position_by_tp`/`close_position_directly` (Task 7), `channel_names.resolve_channel_name` (Task 5), `event_messages.build_sl_hit_message`/`build_external_close_message` (Task 6). `self.event_bus` (new attribute on `TradeManager`, wired in Task 10 — for this task, tests inject a bare `EventBus`-shaped fake or the real one against a temp file, matching the existing `DummyNotifier` convention).
...
Once this plan is merged and deployed, the n8n side still needs manual setup (explicitly out of scope per the spec, §2): a Webhook node at `N8N_EVENT_WEBHOOK_URL`, an IF/Switch node branching on `payload.channel` (actually the top-level `channel` field of the received JSON, not nested under `payload` — the envelope from Task 4 has `channel` as a sibling of `payload`, not inside it; double-check this against spec §4's envelope shape when building the n8n flow), a Data Table insert for both branches, and a Telegram send node using the `message` field verbatim for the `"both"` branch.
